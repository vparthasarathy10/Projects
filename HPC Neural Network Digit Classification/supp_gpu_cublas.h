...
void mat_mul_t(int M, int N, int K, float alpha, float *A, int lda, float *B, int ldb, float beta, float *C, int ldc) {
    ...
    cudaMalloc(&d_A, M * K * sizeof(float));
    cudaMalloc(&d_B, N * K * sizeof(float)); // B is transposed
    cudaMalloc(&d_C, M * N * sizeof(float));

    cublasCreate(&handle);

    cudaMemcpy(d_A, A, M * K * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_B, B, N * K * sizeof(float), cudaMemcpyHostToDevice); // B is transposed
    cudaMemcpy(d_C, C, M * N * sizeof(float), cudaMemcpyHostToDevice);

    cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, N, M, K, &alpha, d_B, ldb, d_A, lda, &beta, d_C, ldc);
...
void t_mat_mul(int M, int N, int K, float alpha, float *A, int lda, float *B, int ldb, float beta, float *C, int ldc) {
    ...
    cudaMalloc(&d_A, K * M * sizeof(float)); // A is transposed
    cudaMalloc(&d_B, K * N * sizeof(float));
    cudaMalloc(&d_C, M * N * sizeof(float));

    cublasCreate(&handle);

    cudaMemcpy(d_A, A, K * M * sizeof(float), cudaMemcpyHostToDevice); // A is transposed
    cudaMemcpy(d_B, B, K * N * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_C, C, M * N * sizeof(float), cudaMemcpyHostToDevice);

    cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, N, M, K, &alpha, d_B, ldb, d_A, lda, &beta, d_C, ldc);
