...
# Function to determine if a given date falls within DST bounds according to
# EU rules
...
for (i in 1:length(att_by_year)) {
  ...
  # Storing all of the plots, one for each year, in list att_forward_plots
  att_forward_plots[[i]] <- p
...
plot <- ggplot(ATT_vs_time, aes(x=Years, y=AttTime)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="red", linewidth = 0.5) +
  labs(x="Year", y="ATT at onset of DST change", title="Trend in Immediate Impact of DST Change on Returns")
